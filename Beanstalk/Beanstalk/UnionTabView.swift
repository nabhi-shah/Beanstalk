    ...
    private var glassTabBar: some View {
        HStack(spacing: 0) {
            ForEach(Array(tabs.enumerated()), id: \.element) { index, tab in
                let isSelected: Bool = {
                    if let dragX = dragX {
                        let tabWidth = 86.0 // Using the hardcoded tab width
                        let tabCenter = (CGFloat(index) * tabWidth) + (tabWidth / 2)
                        let distance = abs(tabCenter - dragX)
...
struct InteractiveSegmentedControl: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: UISegmentedControl, context: Context) {
        ...
        if context.coordinator.lastObservedSwiftUIIndex != selectedIndex {
            context.coordinator.lastObservedSwiftUIIndex = selectedIndex
    ...
    class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        var lastObservedSwiftUIIndex: Int

        init(parent: InteractiveSegmentedControl) {
            self.parent = parent
            self.lastObservedSwiftUIIndex = parent.selectedIndex
        }
        ...
        @MainActor @objc func segmentChanged(_ control: UISegmentedControl) {
            ...
            // Update last observed to prevent cyclical push back to uiView
            lastObservedSwiftUIIndex = index
        ...
        @MainActor @objc func handlePan(_ gesture: UIPanGestureRecognizer) {
            guard let control = gesture.view as? UISegmentedControl, control.numberOfSegments > 0 else { return }
            let location = gesture.location(in: control)
            
            switch gesture.state {
            case .began, .changed:
                parent.dragX = location.x
